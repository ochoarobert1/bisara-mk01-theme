...
function bisara_load_css() {
    ...
    if (!is_admin()){
        ...
        /*- GOOGLE FONTS -*/
        wp_register_style('google-fonts', 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600;700;800&family=Playfair+Display:wght@400;500;600;700;800&display=swap', false, $version_remove, 'all');
        wp_enqueue_style('google-fonts');

        /*- MAIN STYLE -*/
        wp_register_style('main-style', get_template_directory_uri() . '/css/bisara-style.css', false, $version_remove, 'all');
        wp_enqueue_style('main-style');

        /*- WOOCOMMERCE OVERRIDES -*/
        if ( class_exists( 'WooCommerce' ) ) {
            wp_register_style('main-woocommerce-style', get_template_directory_uri() . '/css/bisara-woocommerce.css', false, $version_remove, 'all');
            wp_enqueue_style('main-woocommerce-style');
        }

        /*- MAIN MEDIAQUERIES -*/
        wp_register_style('main-mediaqueries', get_template_directory_uri() . '/css/bisara-mediaqueries.css', array('main-style'), $version_remove, 'all');
...
add_action('wp_enqueue_scripts', 'bisara_load_css');
